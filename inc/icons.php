...
if ( ! class_exists( 'Woostify_Icon' ) ) {
	...
	class Woostify_Icon {
		 ...
		 */
		public static function fetch_svg_icon( $icon = '', $echo = true,$title='' ) {
			$svg_output = '';

			if ( ! self::$woostify_svgs ) {
				ob_start();
				include_once WORDF_DIR . 'assets/svg/svgs.json'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		...
		public static function fetch_all_svg_icon() {
			if ( ! self::$woostify_svgs_arr ) {
				ob_start();
				include_once get_stylesheet_directory() . '/assets/svg/svgs.json'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
